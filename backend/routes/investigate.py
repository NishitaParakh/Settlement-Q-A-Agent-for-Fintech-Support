...
def investigate():
    ...
    if gateway["amount"] != bank["amount"]:
        exceptions.append("Gateway and bank amounts do not match")

    if bank["amount"] != ledger["amount"]:
        exceptions.append("Bank and ledger amounts do not match")

    if not exceptions:
        exceptions.append("No amount mismatch detected")

    # Plain-English explanation
    ai_explanation = (
        f"Transaction {transaction_id} was successful at the gateway "
        f"and bank level, but the ledger is still {ledger['status'].lower()}. "
        f"Therefore, settlement is {settlement_status.lower()}. "
        f"Reason: {reason}."
    )
